...
type RuntimeEnv = { GITHUB_USERNAME?: string; GITHUB_TOKEN?: string; BITBUCKET_USERNAME?: string; BITBUCKET_ACCESS_TOKEN?: string; CONTRIBUTION_EMAILS?: string };
...
export async function GET() {
  ...
  if (env.BITBUCKET_USERNAME && env.BITBUCKET_ACCESS_TOKEN) requests.push(bitbucketContributions(env.BITBUCKET_USERNAME, env.BITBUCKET_ACCESS_TOKEN, (env.CONTRIBUTION_EMAILS ?? "").split(","), range));
  if (!requests.length) return NextResponse.json({ available: false }, { status: 503 });
  const results = await Promise.allSettled(requests);
  const contributions = results.flatMap((result) => result.status === "fulfilled" ? result.value as Awaited<typeof githubContributions> : []);
