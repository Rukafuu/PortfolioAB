...
export const posts: BlogPost[] = [
  {
    id: "seu-agente-nao-e-o-modelo",
    title: {
      pt: "Seu agente não é o modelo. E talvez você esteja benchmarkando a coisa errada.",
      en: "Your agent is not the model. And you may be benchmarking the wrong thing.",
    },
    tag: "AGENTS / EVALS / ENGINEERING",
    excerpt: {
      pt: "O resultado de um coding agent nasce do sistema inteiro: modelo, harness, contexto, ferramentas, ambiente, permissões e feedback.",
      en: "A coding agent's result comes from the whole system: model, harness, context, tools, environment, permissions and feedback.",
    },
    content: {
      pt: "Trocar o modelo do seu coding agent e comemorar +4% no benchmark pode ser o equivalente moderno de trocar o motor enquanto os pneus estão quadrados.\n\nO motor importa.\n\nSó não é o carro inteiro.\n\nQuando um agente abre um repositório, procura arquivos, escolhe o que colocar no contexto, executa comandos, interpreta testes, pede permissão, tenta de novo e finalmente produz um patch, o modelo é apenas uma parte do percurso.\n\nO resultado real nasce de um sistema:\n\nmodelo + harness + contexto + ferramentas + ambiente + permissões + feedback.\n\nMesmo assim, ainda falamos sobre agentes como se estivéssemos comparando modelos isolados.\n\n“O modelo X fez 72%.”\n\n“O modelo Y ganhou quatro pontos.”\n\n“A nova versão programa melhor.”\n\nTalvez.\n\nMas qual harness escolheu os arquivos? Qual ferramenta aplicou o patch? O agente recebeu o resultado completo dos testes? O ambiente tinha as dependências certas? Houve uma segunda tentativa? Ele podia acessar a rede? Quanto contexto foi descartado? E, quando a tarefa parecia concluída, quem decidiu que estava realmente concluída?\n\nSe essas perguntas mudam o resultado, então o objeto que estamos medindo não é apenas o modelo.\n\n## SWE-bench mudou a conversa — e isso foi extremamente importante\n\nAntes do SWE-bench, boa parte da avaliação de código ainda parecia uma prova de programação: uma descrição curta, uma função pequena e uma resposta comparada com testes previsíveis.\n\nO SWE-bench levou a avaliação para repositórios reais. Sua versão original reuniu 2.294 problemas derivados de issues e pull requests em 12 projetos Python. Para resolver uma tarefa, não bastava completar uma função. Era necessário navegar por uma codebase, entender relações entre arquivos, editar o projeto e fazer os testes passarem.\n\nIsso foi uma mudança enorme.\n\nO benchmark ajudou a transformar “o modelo consegue gerar código?” em uma pergunta muito mais útil:\n\n“O sistema consegue resolver uma issue real?”\n\nNa publicação original, até o melhor resultado era muito baixo. Isso expôs com clareza o tamanho da distância entre gerar snippets convincentes e trabalhar dentro de software existente.\n\nO problema não é que o SWE-bench tenha ficado ruim.\n\nO problema é tratá-lo como resposta universal para uma indústria que cresceu ao redor dele.\n\nResolver bugs continua sendo central. Mas engenharia de software também envolve escrever testes que realmente diferenciam implementações, responder a review, manter estilo, investigar regressões, migrar dependências, lidar com ambientes quebrados e reconhecer quando a melhor alteração é não alterar nada.\n\nUma única taxa de resolução não consegue explicar tudo isso.\n\n## O mesmo modelo pode produzir agentes diferentes\n\nImagine dois agentes usando exatamente o mesmo modelo.\n\nO primeiro recebe o README e os vinte arquivos que uma busca semântica considerou mais próximos da issue. Ele pode editar arquivos, mas não executar a aplicação. Ao terminar, roda um comando de teste genérico e entrega o diff.\n\nO segundo começa lendo as instruções do repositório, identifica o pacote afetado, reproduz o erro, segue imports e consumidores, executa testes direcionados, observa a falha, aplica uma mudança mínima e verifica o resultado novamente. Se o ambiente estiver incompleto, ele distingue falha de infraestrutura de falha causada pelo patch.\n\nMesmo modelo.\n\nMesma tarefa.\n\nComportamentos completamente diferentes.\n\nO primeiro pode parecer mais rápido e barato. O segundo pode gastar mais tokens e ainda assim produzir um resultado muito mais confiável. Em outro tipo de tarefa, a busca ampla do primeiro pode encontrar uma relação que o fluxo dirigido do segundo ignorou.\n\nNão existe harness neutro.\n\nCada escolha altera o que o modelo consegue observar, quais ações consegue executar e qual feedback recebe antes de decidir o próximo passo.\n\nO harness não é embalagem.\n\nEle participa da inteligência aparente.\n\n## Contexto é uma política, não um balde\n\nÉ comum falar sobre janela de contexto como se mais espaço resolvesse o problema.\n\nMais tokens permitem carregar mais arquivos. Mas alguém — ou alguma coisa — ainda precisa decidir quais arquivos entram, em que ordem, com quais trechos e por quanto tempo.\n\nEsse processo já contém uma hipótese sobre o sistema.\n\nSe o agente recebe apenas arquivos com nomes parecidos com a issue, pode perder um consumidor distante. Se recebe o repositório inteiro, relações importantes passam a disputar atenção com milhares de detalhes irrelevantes. Se o resumo de uma etapa anterior estiver errado, mais contexto apenas torna o erro mais bem documentado.\n\nContexto não é quantidade bruta.\n\nÉ seleção, hierarquia, atualização e descarte.\n\nUm bom agente precisa saber buscar mais quando a evidência é insuficiente e esquecer quando o material antigo deixou de ajudar. Precisa separar documentação declarada, comportamento observado, histórico e suposição.\n\nQuando um benchmark não registra como o contexto foi construído, ele esconde uma parte essencial do experimento.\n\n## Ferramentas definem o que “saber programar” significa\n\nUm modelo pode descrever perfeitamente o comando que deveria executar.\n\nIsso não significa que o comando será executado.\n\nA ferramenta pode truncar a saída, perder o código de erro, bloquear um processo longo ou devolver apenas os últimos caracteres de um log. Um editor pode aplicar o patch em um arquivo diferente. Uma busca pode ignorar arquivos não indexados. Um terminal pode manter estado entre chamadas ou reiniciar a cada ação.\n\nEsses detalhes parecem infraestrutura até mudarem o resultado.\n\nO mesmo vale para permissões.\n\nUm agente com rede, banco, credenciais e shell irrestrito talvez resolva mais tarefas. Também pode produzir um benchmark impossível de reproduzir e um produto impossível de confiar.\n\nUm agente preso em um sandbox extremamente rígido pode parecer menos capaz, quando na verdade está operando sob um contrato de segurança diferente.\n\nCapacidade sem registrar autoridade é uma comparação incompleta.\n\n## Feedback é parte do raciocínio\n\nAgentes não trabalham apenas com o prompt inicial.\n\nEles trabalham com o que volta do mundo.\n\nUm teste falha e revela uma suposição errada. O compilador aponta um contrato quebrado. O linter mostra que o patch atingiu outro pacote. Um reviewer explica por que uma solução aparentemente correta não respeita a arquitetura.\n\nEsse feedback muda a trajetória.\n\nSe o harness esconde stderr, resume demais os testes ou marca sucesso depois do primeiro comando verde, o modelo raciocina sobre uma realidade empobrecida.\n\nTambém existe o problema oposto: agentes que executam dezenas de verificações sem transformar os resultados em uma decisão. Eles acumulam observações, mas não fecham o ciclo.\n\nPor isso, avaliar apenas o patch final perde informações importantes.\n\nO agente localizou o problema certo?\n\nReproduziu a falha?\n\nEscolheu uma verificação capaz de refutar sua hipótese?\n\nPercebeu quando o ambiente estava quebrado?\n\nSoube parar?\n\nUm score binário diz se o teste final passou. Não diz por que o sistema chegou ali — nem se chegaria novamente em uma codebase ligeiramente diferente.\n\n## O paper de junho colocou o problema no nome certo\n\nEm junho de 2026, o position paper “Coding Benchmarks Are Misaligned with Agentic Software Engineering” formulou a crítica de maneira direta: benchmarks atuais colapsam modelo, harness e ambiente em um único score de ponta a ponta.\n\nOs autores descrevem um coding agent como um sistema composto por modelos, harnesses, contextos, ambientes e sinais de feedback. Também apontam dois problemas adicionais: avaliar contra uma única solução de referência pode penalizar alternativas válidas, e um score final sem sinais por componente oferece pouca orientação para melhorar o sistema.\n\nEssa segunda parte é especialmente importante.\n\nSe o score sobe quatro pontos depois de trocar o modelo, sabemos que o sistema melhorou naquele benchmark.\n\nMas se o score cai, o que falhou?\n\nLocalização?\n\nSeleção de contexto?\n\nUso de ferramenta?\n\nInterpretação de feedback?\n\nVerificação?\n\nSem medir o ciclo, toda otimização vira tentativa e erro com uma etiqueta científica.\n\n## OmniCode amplia o que consideramos trabalho de engenharia\n\nO OmniCode segue outra direção útil. Em vez de concentrar toda a avaliação em correção de bugs Python, ele reúne 1.794 tarefas em Python, Java e C++, distribuídas entre quatro categorias: bug fixing, geração de testes, correção após code review e correção de estilo.\n\nEssa ampliação produz um resultado intuitivo, mas fácil de esquecer olhando apenas um leaderboard: desempenho não transfere de forma uniforme.\n\nUm agente forte em bug fixing Python pode cair bastante quando precisa gerar um teste que diferencie uma implementação correta de uma errada. Pode funcionar bem em uma linguagem e falhar em outra porque o build, as ferramentas e as convenções mudaram.\n\nIsso não significa apenas que o modelo “sabe menos Java”.\n\nPode significar que o harness foi amadurecido ao redor de pytest, que a estratégia de contexto aprendeu padrões de projetos Python, que o feedback do compilador Java é tratado pior ou que a imagem de execução possui um caminho mais frágil.\n\nQuando a tarefa muda, o sistema inteiro é testado de outro jeito.\n\n## Benchmark de sistema, não propaganda de modelo\n\nSe agentes são sistemas, os relatórios deveriam tratá-los assim.\n\nUm resultado útil precisa registrar pelo menos:\n\n* modelo e versão\n* harness e configuração\n* estratégia de contexto\n* ferramentas disponíveis\n* ambiente e dependências\n* permissões e acesso à rede\n* orçamento de tokens, custo e tempo\n* política de tentativas e recuperação\n* verificações executadas\n* critério de conclusão\n\nIsso não torna rankings inúteis.\n\nTorna-os honestos.\n\nO nome do modelo continua importante, mas deixa de ocupar sozinho o título de um experimento produzido por dezenas de outras decisões.\n\nTambém permite comparar o que realmente interessa. Talvez um sistema resolva dois pontos percentuais a menos, mas custe metade, termine três vezes mais rápido e produza patches menores. Talvez outro seja excelente em tarefas bem especificadas e péssimo quando precisa descobrir o contrato real da codebase.\n\nUm número único esconde essas escolhas.\n\n## O benchmark que mais importa pode estar dentro da sua codebase\n\nBenchmarks públicos ajudam a acompanhar fronteiras, comparar ideias e impedir que toda demonstração seja escolhida para favorecer o produto.\n\nMas eles não conhecem seu monorepo.\n\nNão conhecem aquele serviço legado sem testes, a convenção que vive em um arquivo de instruções, o deploy que depende de duas equipes, a regra de privacidade que proíbe enviar determinado arquivo ou o módulo em que “refatoração pequena” significa três dias de regressão.\n\nPara escolher um agente de trabalho, o melhor benchmark pode ser um conjunto versionado de tarefas da própria codebase.\n\nIssues antigas com solução conhecida.\n\nBugs que exigem atravessar camadas.\n\nMudanças pequenas com risco alto.\n\nTestes que precisam ser escritos antes do fix.\n\nReviews em que uma solução plausível viola uma decisão arquitetural.\n\nTarefas em que o agente deveria pedir permissão.\n\nE tarefas em que deveria concluir que não possui evidência suficiente para alterar nada.\n\nMeça resolução, mas também tamanho do diff, regressões, custo, tempo, intervenções humanas, comandos perigosos bloqueados e qualidade da justificativa. Preserve os traces necessários para entender falhas sem transformar pensamento privado em requisito de auditoria.\n\nEsse benchmark local não substitui o SWE-bench ou o OmniCode.\n\nEle responde a uma pergunta diferente:\n\n“Este sistema trabalha bem no software que nós realmente mantemos?”\n\n## Talvez o próximo salto não venha do próximo modelo\n\nModelos continuarão melhorando.\n\nE cada geração provavelmente entregará ganhos reais em raciocínio, código e uso de ferramentas.\n\nMas coding agents já estão bons o suficiente para expor os limites ao redor do modelo.\n\nÀs vezes, o próximo salto vem de contexto melhor selecionado.\n\nDe uma ferramenta que preserva o erro completo.\n\nDe um sandbox que permite o necessário sem liberar tudo.\n\nDe testes executados no momento certo.\n\nDe memória de projeto que separa fato, intenção e hipótese.\n\nDe um critério de conclusão que exige evidência em vez de confiança.\n\nTrocar o motor ainda pode deixar o carro mais rápido.\n\nMas, antes de comemorar os quatro pontos, talvez valha verificar os pneus.\n\n## Referências\n\n* [SWE-bench: Can Language Models Resolve Real-World GitHub Issues?](https://arxiv.org/abs/2310.06770)\n* [Position: Coding Benchmarks Are Misaligned with Agentic Software Engineering](https://arxiv.org/abs/2606.17799)\n* [OmniCode: A Benchmark for Evaluating Software Engineering Agents](https://arxiv.org/abs/2602.02262)",
      en: "Replacing the model in your coding agent and celebrating a four-point benchmark gain may be the modern equivalent of changing the engine while the tires are square.\n\nThe engine matters. It is simply not the whole car.\n\nWhen an agent opens a repository, searches for files, selects context, runs commands, interprets tests, requests permission, retries and finally produces a patch, the model is only one part of the path.\n\nThe real result comes from a system:\n\nmodel + harness + context + tools + environment + permissions + feedback.\n\nYet we still talk about agents as if we were comparing isolated models.\n\n## SWE-bench changed the conversation\n\nSWE-bench moved code evaluation from short programming exercises into real repositories. Its original release collected 2,294 problems from issues and pull requests across 12 Python projects. Solving them required navigating a codebase, coordinating changes and using an execution environment.\n\nThat was a major shift. It replaced “can the model generate code?” with a more useful question: “can the system resolve a real issue?”\n\nSWE-bench remains valuable. The mistake is treating it as a universal answer for an industry that grew around it. Software engineering also includes generating meaningful tests, responding to review, maintaining style, investigating regressions and deciding when no change should be made.\n\nOne resolution rate cannot explain all of that.\n\n## The same model can produce different agents\n\nImagine two agents using the exact same model.\n\nThe first receives a README and twenty files selected by semantic similarity. It can edit but not run the application. At the end, it executes a generic test command and returns the diff.\n\nThe second reads repository instructions, identifies the affected package, reproduces the failure, follows imports and consumers, runs targeted tests, applies a minimal change and verifies the result again. It distinguishes infrastructure failures from regressions caused by its patch.\n\nSame model. Same task. Completely different behavior.\n\nThere is no neutral harness. Every choice changes what the model can observe, what it can do and what feedback it receives. The harness is not packaging. It participates in apparent intelligence.\n\n## Context is a policy, not a bucket\n\nA larger context window does not decide which files enter, in what order, at what granularity or for how long.\n\nIf an agent sees only files with names similar to the issue, it may miss a distant consumer. If it sees the entire repository, important relationships compete with thousands of irrelevant details. If an earlier summary is wrong, more context merely documents the mistake more thoroughly.\n\nContext means selection, hierarchy, refresh and deletion. A benchmark that does not record how context was assembled hides an essential part of the experiment.\n\n## Tools define what programming ability means\n\nA model may describe the correct command perfectly. That does not mean the command will run correctly.\n\nA tool can truncate output, lose an exit code, apply a patch to the wrong file or return only the tail of a log. A sandbox may block a necessary operation. Another agent may have network, credentials and an unrestricted shell, solving more tasks under a security contract that would be unacceptable in production.\n\nCapability without recorded authority is an incomplete comparison.\n\n## Feedback is part of reasoning\n\nAgents work with what comes back from the world. A failing test exposes a false assumption. A compiler reveals a broken contract. A reviewer explains why a plausible patch violates the architecture.\n\nIf the harness hides stderr, over-summarizes tests or declares success after the first green command, the model reasons over an impoverished reality.\n\nThis is why the final patch is not the entire evaluation. Did the agent localize the right problem, reproduce the failure, choose a check capable of disproving its hypothesis, recognize a broken environment and know when to stop?\n\n## The June paper names the problem\n\nThe June 2026 position paper “Coding Benchmarks Are Misaligned with Agentic Software Engineering” argues that current benchmarks collapse model, harness and environment into a single end-to-end score.\n\nIt describes a coding agent as a composite of models, harnesses, contexts, environments and feedback signals. It also argues that a single reference solution may penalize valid alternatives, while an end-to-end score without component-level signals gives teams little guidance about what to improve.\n\nIf a score drops, what failed: localization, context selection, tool use, feedback interpretation or verification?\n\nWithout measuring the cycle, optimization becomes trial and error with a scientific label.\n\n## OmniCode broadens the job\n\nOmniCode includes 1,794 tasks across Python, Java and C++, covering bug fixing, test generation, code review fixing and style fixing.\n\nIts results reinforce an intuitive point: performance does not transfer uniformly. An agent that is strong at Python bug fixing may struggle when it must generate a discriminating test or operate inside a different language ecosystem.\n\nThat does not necessarily mean only that the model “knows less Java.” The harness may have been optimized around pytest, the context strategy may favor Python repositories, or compiler feedback may be handled poorly.\n\nWhen the task changes, the whole system is tested differently.\n\n## Benchmark systems, not model marketing\n\nA useful report should record the model and version, harness configuration, context strategy, tools, environment, permissions, network access, token budget, cost, time, retry policy, verification steps and completion criteria.\n\nThis does not make leaderboards useless. It makes them honest.\n\nIt also reveals the tradeoffs that one score hides. A system may solve slightly fewer tasks while costing half as much, finishing much faster and producing smaller patches.\n\n## Your most important benchmark may live in your codebase\n\nPublic benchmarks are essential for tracking the frontier, but they do not know your monorepo, legacy service, privacy constraints, architectural conventions or deployment risks.\n\nFor choosing a working agent, build a versioned suite from your own history: old issues with known solutions, cross-layer bugs, high-risk small changes, test-first tasks, review corrections, permission boundaries and tasks where the right answer is to avoid changing anything.\n\nMeasure resolution, but also diff size, regressions, cost, time, human interventions, blocked dangerous commands and evidence quality.\n\nThat local benchmark does not replace SWE-bench or OmniCode. It answers a different question:\n\n“Does this system work well in the software we actually maintain?”\n\nModels will continue to improve. But coding agents are already good enough to expose the limits around the model.\n\nSometimes the next leap comes from better context, complete tool feedback, a safer sandbox, project memory or a completion criterion that requires evidence instead of confidence.\n\nChanging the engine may still make the car faster.\n\nBefore celebrating the four points, check the tires.\n\n## References\n\n* [SWE-bench: Can Language Models Resolve Real-World GitHub Issues?](https://arxiv.org/abs/2310.06770)\n* [Position: Coding Benchmarks Are Misaligned with Agentic Software Engineering](https://arxiv.org/abs/2606.17799)\n* [OmniCode: A Benchmark for Evaluating Software Engineering Agents](https://arxiv.org/abs/2602.02262)",
    },
