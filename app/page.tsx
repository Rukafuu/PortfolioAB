...
function scheduledLabel(date: string, language: Language) {
  ...
  return language === "pt"
    ? `${String(day).padStart(2, "0")} ${monthLabel} / EM BREVE`
    : `${monthLabel} ${String(day).padStart(2, "0")} / SOON`;
}
...
const featuredProjects = [
  ...
  { id: "04", name: "AAG Agent Protocol", repo: "AAG-Protocol", url: "https://github.com/Rukafuu/AAG-Protocol", type: "PROTOCOL / JSON SCHEMA / AGENT EVALS", status: "DRAFT 0.2",
    description: { pt: "Especificação aberta para agentes criarem mapas estruturais verificáveis de repositórios, com regras de conformidade e benchmark experimental.", en: "An open specification for agents to build verifiable repository maps, with conformance rules and an experimental benchmark." } },
  { id: "05", name: "TimeWarp", repo: "TimeWarp", url: "https://github.com/Rukafuu/TimeWarp", type: "GO / SQLITE / MCP / OBSERVABILITY", status: "NEW",
    description: { pt: "Ferramenta de observabilidade e debugging que grava evidências de execução, reconstrói cadeias causais e suporta replay para inspeção via CLI/MCP. TimeWarp mostra o que aconteceu — não decide o que deve acontecer.", en: "An observability and debugging tool that records execution evidence, reconstructs causal chains and supports replay for inspection via CLI/MCP. TimeWarp tells you what happened — it does not decide what should happen next." } },
  { id: "06", name: "Ketchup", repo: "Ketchup", url: "https://github.com/Rukafuu/Ketchup", type: "GO / DEVELOPER TOOLING", status: "ACTIVE",
    description: { pt: "Ferramenta de retomada de contexto para desenvolvedores. Responde à pergunta: 'O que mudou enquanto eu estava fora?'. Never run out of sauce.", en: "A developer context resumption tool. Answers the question: 'What changed while I was away?'. Never run out of sauce." } },
  { id: "07", name: "Tsuma", repo: "tsuma", url: "https://github.com/Rukafuu/tsuma", type: "ELECTRON / THREE.JS / VRM / MCP", status: "ALPHA",
    description: { pt: "Companheira desktop para agentes de codificação. Usa MCP para traduzir estado/eventos do agente em comportamento visual e animação. Tsuma não gera inteligência — ela visualiza.", en: "A desktop companion for coding agents. Uses MCP to translate agent state/events into visual behavior and animation. Tsuma doesn't generate intelligence — she visualizes it." } },
  { id: "08", name: "GhostClock", repo: "GhostClock", url: "https://github.com/Rukafuu/GhostClock", type: "C17 / WIN32 / WINDOWS INTERNALS", status: "EXPERIMENTAL",
    description: { pt: "Ferramenta experimental de orquestração de desempenho para Windows focada em otimizações mensuráveis e reversíveis sem alterar clocks de CPU ou GPU. C17, Win32, Windows internals.", en: "An experimental Windows performance orchestration tool focused on measurable, reversible optimizations without changing CPU or GPU clocks. C17, Win32, Windows internals." } },
];
...
const archiveProjects = [
  ...
  { name: "ExpoMCP", repo: "ExpoMCP", type: "TYPESCRIPT / MCP / EXPO", description: { pt: "Servidor MCP local e somente leitura para agentes inspecionarem e diagnosticarem projetos React Native/Expo sem expor segredos.", en: "A local read-only MCP server that lets agents inspect and diagnose React Native/Expo projects without exposing secrets." } },
  ...
  { name: "LiraOS", repo: "LiraOS", type: "PYTHON / DESKTOP AI", description: { pt: "Uma geração anterior do ecossistema de assistentes Lira — projeto histórico/congelado.", en: "An earlier generation of the Lira assistant ecosystem — historical/frozen project." } },
...
const copy = {
  pt: {
    ...
    nowBody:
      "Atualmente construindo TimeWarp, Ketchup, Tsuma e GhostClock. Trabalhando com backend, IA e agent infrastructure. Criando software open source na wAIfu Corp e escrevendo sobre as partes humanas de criar tecnologia.",
  ...
  en: {
    ...
    nowBody:
      "Currently building TimeWarp, Ketchup, Tsuma and GhostClock. Working on backend, AI and agent infrastructure. Creating open-source software at wAIfu Corp and writing about the human parts of making technology.",
